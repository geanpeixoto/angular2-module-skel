import { CallbackFn } from './tasks-helper';
  ...
  return Promise.all(globs.map(pattern => {
    ...
    return Promise.all(files.map(filePath => {
      return readFile(filePath, 'utf-8')
        .then(content => inlineResourcesFromString(content, (url: string) => {
 ...
 */
export function inlineResourcesFromString(content: string, urlResolver: any) {
